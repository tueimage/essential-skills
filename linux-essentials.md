...
8. After it completes you can optionally let the installer create icons on the Desktop and Start Menu.
...
A black screen with white text (or a white screen with black text if your using macOS) will appear. There is probably a blinking cursor and a `$`-sign visible on the screen. There may be some text in front of the dollar sign like the user name or computer name that you can safely ignore for now. The `$`-sign is called the Bash 'prompt'. Bash is the program that you run the commands in. From this prompt you can type commands. When your command is finished you can press enter to see the result, which will be printed below the command you typed.
...
Below, that you get a new prompt on which you can enter new commands. If you type `date` it will display the current date and time below that.
...
We are going to setup a secure key that you can use to login to the server. A key is basically a super long password that will be used to let the server that it is really you who wants access. The key consists of two parts: a public key and a private key. The private key is on your own computer. The public key will be put on the server. 
...
The administrator will now make sure the key will be known by the server. Once you get a confirmation this is done, you can login to the server.
...
This will let the user `amalia` login to the server at the address `saruman.bmt.tue.nl` using the secure shell. Of course, you will have to replace this with your name and the correct address for the server you want to use. The `-X` is there to make sure you can view plots and figures on the server (see `Running Python on the server`).

You will now see some text that indicates your login was succesful, which machine you are connected to and which operating system it uses. Depending on the operating system, there may be a whole lot more text that you can safely ignore for now.

Below all that text is a new prompt. By default, it will look like this:

```bash
amalia@saruman:~$
```

From this prompt you can again type `whoami` or `date`. Now, these commands will run on the server. The results will be send back to your own PC and show up on the screen like normal.
...
In the directory you can inspect files and folders, run programs, and create, move, and delete files or folders. If you want to see what the content of the current directory are, you use the `ls` command, and it will show you a list of the contents in the folder. There is a chance that this folder is currently empty, so it will probably show nothing. Let's change that by typing `mkdir my_first_folder`. `mkdir` stands for 'make directory'. If you type `ls` again, you will see it shows the new folder:
...
Wonderful. Now you can navigate to this folder by typing `cd my_first_folder`. Note that the prompt changes to reflect you new working directory.
...
Note, that gedit runs as graphical application, which does not always work well. There are also editors with text-based interfaces that run in the Terminal window, such as `nano`, `pico`, and `vim`. `nano` and `pico` can be operated using the shortkeys shown at the bottom of the window (^ means <kbd>Ctrl</kbd>), for example to exit the editor (<kbd>Ctrl+X</kbd>) or save the file (<kbd>Ctrl+O</kbd>). 

`vim` is a story in itself, because it is a very powerful editor which can be operated using a small programming language and therefore has a very steep learning curve. Should you ever open it by accident, know that you can quit it by pressing <kbd>Esc</kbd> a few times, and typing `:q` followed by enter. Makes sense right?

---

Now your file contains some text, you can show it in the Terminal using the `cat` or `more` commands (`cat my_first_file.txt`). The `more` command shows the file page-by-page that you can page through using the <kbd>Enter</kbd> key.
...
Tip
: The <kbd>Tab</kbd>-key can be used for autocompletion of file paths. If you for example type `cd ~/my_` followed by <kbd>Tab</kbd> the terminal will give suggestions for completion of your line. This can save a lot of time!
...
If you want to copy a folder with their content, you use the `cp` command with a `-r` flag, which means `recursive`, i.e. also copy everything *inside* the folder
...
For this you can use the `scp` command, which stands for secure copy. Instead of only specifying filenames, you specify from or to which computer as well. In any way, you *need* to run this command from a terminal window *on your PC*, so without logging into the server. You can get a new terminal window from the terminal application's menu, which will run on your PC.
...
**Be careful!** When moving a folder to a destination folder that does not exist yet, the folder's name will be changed instead. If the destination *does* exist, the folder will be *moved* to the destination folder.
...
This shows quite some information, that you don't need to understand all at this point. Let's break it down to give an idea of what it means for `my_first_folder`.

* `drwxrwxr-x` are the permissions of the file or folder. In this case, the `d` shows you that it is a folder (folders are probably also given a distinct color). For files both the color and the `d` are absent.
* The permissions can be broken into three groups of three: `rwx`, `rwx`, and `r-x`. 
...
Surprisingly, `ls` or `ls -l(h)` does not show everything. If you want to show all the hidden stuff in your home folder, include the `-a` flag, like this:
```bash
...
Your result may be include other stuff as well. The files starting with dots are invisible by default because they are configuration files. The `.bash_history` file contains the history of the commands you have typed. `.bashrc` and `.profile` contain some settings for your account. Normally you do not have to touch them, but it is good to know these files exist. If you look at the permissions, you can see that other users can only read the files, because of the `r--` in the 'other users' permissions. You also see that `.` and `..` appear here as links which are followed when using `cd ..` to go to the folder one level-up, or when using `.` to refer to the current folder.
...
Clearly, these folders are not that big: only 8 kilobytes. If you have large data sets, `du -sh` can display M or G to denote megabytes and gigabytes.
...
Hopefully, you now know how to get your data and code on the server, and how to organize it once you have done so. Now, you will want to run that code. In this section, we are working with an Python script that you can save on the server. To run the examples, make a new folder in your home folder called `process_example`, and create a file called `my_clock.py`. Open the file in your favorite editor, and write the following code:
...
The script is executed by the Python runtime. Technically, that is a program that runs the2 script for you. Linux will give this runtime a so-called process ID which you can use to identify your script. Notice that the script is completly modal: you can't do anything else in the Terminal window until you quit it. Of course, you can open a new Terminal window and make a connection to the server in another window to do other things though.
...
Of course, this script is not the most useful, but the same thing works for large deep learning scripts that can take quite a lot of computation time. At some point you want to stop it from running.

---

Pro-tip
:   Running the `python` command with the `-i` flag runs the script in so-called 'interactive' mode. When you press <kbd>Ctrl-C</kbd>, the Python runtime will send a `KeyboardInterrupt`, but not quit (!). It returns to the Python interpreter (with a `>>>` prompt) with all classes, functions, variables still in memory. This is also useful when debugging, because when the script crashes, you can use the interpreter to assess what went wrong. You can try it out on the script above, and inspect what the value of `now` is.
...
Sometimes it can be useful to pause a program temporarily, do something else, and continue with it. You can press <kbd>Ctrl-Z</kbd> for this. It will put the current program in a sort of hibernation state. You can do other things in the Terminal. You can continue the program with the `fg` command. Note that the Terminal will print the process id of the current program when you pause it:
...
If a script or program has crashed so hard it does not even respond to <kbd>Ctrl-C</kbd>, you can use <kbd>Ctrl-\</kbd> which will stop *everything associated with a process*. This is particularly useful when your script uses multiple threads (e.g. the multiprocessing library in Python). Don't worry about this if you have no clue what it is.
...
The first step is to check which GPUs are available. The TU/e servers have a tool called `nvtop` to check this:
...
At the moment, GPUs 0 and 2 are occupied (indexes start at 0). As you can see, the tool prints the commands with the user that put it there, the process ID, the amount of CPU and RAM it is using, and, most importantly, how much GPU memory is used. Currently, GPU 1 is doing nothing (as indicated by `No processes`), which means that there is nothing stopping you from using it.
...
Run it with `python -i` to make sure that the Python interpreter keeps running after the script -- which only takes a few seconds to run -- stays alive. Pause the interpreter with <kbd>Ctrl-Z</kbd> and look at `nvtop` to see if you see if your script appears in the list for the GPU you selected. If it does not, you made a mistake somewhere. If it is on all GPUs you are doing something very wrong. 
